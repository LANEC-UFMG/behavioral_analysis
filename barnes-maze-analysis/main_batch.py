...
for it in range(len(filename)):
    ...
    position_each_hole = get_hole_positions(df, filename[it], prev_hole_position=prev_hole_position) 
    ...
    #plt.plot(quadrant_dict["quadrant1"][:,0], -quadrant_dict["quadrant1"][:,1])
    #plt.plot(quadrant_dict["quadrant2"][:,0], -quadrant_dict["quadrant2"][:,1])
    #plt.plot(quadrant_dict["quadrant3"][:,0], -quadrant_dict["quadrant3"][:,1])
    #plt.plot(quadrant_dict["quadrant4"][:,0], -quadrant_dict["quadrant4"][:,1])
    #######################
    ...
    if latency_mode == 'escape':
        ...
        latency, body_part_matrix_nose, body_part_matrix_body_centre, bp_pos_on_maze, bp_pos_on_maze_filtered\
            = get_trial_latency_to_find_scape(body_part_matrix_nose, body_part_matrix_body_centre, bp_pos_on_maze, bp_pos_on_maze_filtered, time_thresh=0.1, fps=fps, scape_hole=1)
    ...
    inst_speed, inst_speed_entire, av_speed, av_speed_bins = get_inst_speed_bins(body_part_matrix_body_centre, maze_info_pixel, time_win=10, fps=30)
    ...
    ratio_target_others, time_on_each_quadrant, ratio_target_minute = get_time_on_target_quadrant(bp_pos_on_quadrant, target=target_quadrant, fps=30, prob=prob, trial_duration=trial_duration)
    ...
    basename_parts = ''.join(basename).split('_')
    box = re.sub('\D','',basename_parts[0])
    ID = re.sub('\D','',basename_parts[1])
    group = re.sub('\D','',basename_parts[2])
    day = re.sub('\D','',basename_parts[3])
    trial = re.sub('\D','',basename_parts[4])
    basename_identifier = 'B' + box + '_' + ID + '_G' + group + '_D' + day + '_T' + trial
    
    # Create a data frame to append to the final dataframe
    data = pd.DataFrame([[box, ID, group, day, trial, beg, end, latency, p_errors, s_errors, 
                          strategy, total_distance, av_speed, ratio_target_others, ratio_target_minute,
                          time_on_each_quadrant[0], time_on_each_quadrant[1], time_on_each_quadrant[2], time_on_each_quadrant[3],
                          av_speed_bins]], 
                        columns = ['Box','ID','Group','Day','Trial','Beginning', 'End', 'Latency', 'P_error', 'S_error',
                                       ...
                                       'Time_on_quadr_0', 'Time_on_quadr_1', 'Time_on_quadr_2', 'Time_on_quadr_3', 'Av_speed_bins']) 
    
        
    # makes index continuous
    trial_info = pd.concat([trial_info, data], ignore_index = True) 
    # Count the number of times each hole was visited
    data_holes = get_head_dips(bp_pos_on_maze_filtered, box, ID, group, day, trial, 
                                   interval = 30, trial_duration = trial_duration, fps=fps, probe=True)
    ...
    save_filename = os.path.dirname(filename[it])+'/'+basename_identifier+'.txt'
    ...
    save_filename_plot = os.path.dirname(filename[it])+'/'+basename_identifier
    big_plot(body_part_matrix_nose, body_part_matrix_body_centre, bp_pos_on_maze_filtered, centroid_coords, position_each_hole, maze_info_pixel, inst_speed_entire, quadrant_dict, bp_pos_on_quadrant, save_filename_plot, trial_data=data, trial_name=basename_identifier, show=False)
    ...
    print(str(it)+' - '+basename_identifier + str(': OK!'))
...
if prob:
    trial_or_probe = 'probe_'
else:
    trial_or_probe = ''
    
fname = 'Final_results_' + trial_or_probe + 'R' + ID + group_sufix

# Save as h5 and xlsx 
save_filename = os.path.dirname(filename[it])+'/'+fname+'.h5'
#save_filename = os.path.dirname(filename[it])+'/'+'Final_results_probe'+'.h5'
trial_info.to_hdf(save_filename, key='trial_info', mode='w')  
trial_info = pd.read_hdf(save_filename, key='trial_info') 

save_filename = os.path.dirname(filename[it])+'/'+fname+'.xlsx'
trial_info.to_excel(save_filename)  
trial_info = pd.read_excel(save_filename)

# Save the holes visited as h5 and xlsx
save_filename = os.path.dirname(filename[it])+'/'+fname+'_holes.xlsx'
hole_visits_df.to_excel(save_filename)  
hole_visits_df = pd.read_excel(save_filename)

save_filename = os.path.dirname(filename[it])+'/'+fname+'_holes.h5'
hole_visits_df.to_hdf(save_filename, key='hole_visits_df', mode='w')  
hole_visits_df = pd.read_hdf(save_filename, key='hole_visits_df') 
