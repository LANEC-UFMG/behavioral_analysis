...
# Function to extract the animals instantaneous speed
def get_inst_speed_bins(body_part_matrix, maze_info_pixel, time_win=1, fps=30):
    # Get the distance frame by frame (only the first output)
    euc_distance_frame = get_distance(body_part_matrix, maze_info_pixel)[0]
    # Time window in sec
    time_win_sec = time_win/fps
    ...
    # Mean speed per bins of 30 sec
    av_speed_on_each_bin = np.zeros(4,)
    # Get the time on each quadrant
    av_speed_on_each_bin[0] = np.mean(inst_speed[0:np.int64(30*fps/time_win)])
    av_speed_on_each_bin[1] = np.mean(inst_speed[np.int64(30*fps/time_win):np.int64(60*fps/time_win)])
    av_speed_on_each_bin[2] = np.mean(inst_speed[np.int64(60*fps/time_win):np.int64(90*fps/time_win)])
    av_speed_on_each_bin[3] = np.mean(inst_speed[np.int64(90*fps/time_win):np.int64(120*fps/time_win)])
    
    return inst_speed, inst_speed_entire, av_speed, av_speed_on_each_bin
...
def get_time_on_target_quadrant(bp_pos_on_quadrant, target=1, fps=30, prob=False, trial_duration=2):
    ...
    # Convert trial duration from minutes to seconds
    trial_duration_sec = trial_duration*60
    ...
    ratio_target_half_minute = np.zeros((4,))
    
    # Also calculate the time by minute of trial, if it's a probe one  
    if prob is True:
        ...
        # Limit the probe trial duration to 'trial_duration' minutes
        time_on_each_quadrant[0] = np.count_nonzero(bp_pos_on_quadrant[0:trial_duration_sec*fps-1] == 1) / fps
        time_on_each_quadrant[1] = np.count_nonzero(bp_pos_on_quadrant[0:trial_duration_sec*fps-1] == 2) / fps
        time_on_each_quadrant[2] = np.count_nonzero(bp_pos_on_quadrant[0:trial_duration_sec*fps-1] == 3) / fps
        time_on_each_quadrant[3] = np.count_nonzero(bp_pos_on_quadrant[0:trial_duration_sec*fps-1] == 4) / fps
        
        # ------------------ Bruno
        # Calculate the time in each quadrant, but in bins of 30 secs
        target = 1
        a = bp_pos_on_quadrant[0:30*fps-1]
        ratio_target_half_minute[0] = np.count_nonzero(a == target) / (30*fps)
        
        b = bp_pos_on_quadrant[30*fps:60*fps]
        ratio_target_half_minute[1] = np.count_nonzero(b == target) / (30*fps)
        
        b = bp_pos_on_quadrant[60*fps:90*fps]
        ratio_target_half_minute[2] = np.count_nonzero(b == target) / (30*fps)
        
        b = bp_pos_on_quadrant[90*fps:120*fps]
        ratio_target_half_minute[3] = np.count_nonzero(b == target) / (30*fps)
        
    #### ---------------------- Bruno
    # Return the time spent in each quadrante every 30 seconds, not 1 min
    # return ratio_target_others, time_on_each_quadrant, ratio_target_minute
    return ratio_target_others, time_on_each_quadrant, ratio_target_half_minute
    # ------------------------

# Get the number of head dips in each hole in intervals of "interval" seconds
def get_head_dips(bp_pos_on_maze_filtered, box, ID, group, day, trial, interval=30, trial_duration=2, fps=30, probe=False):
    holes_visited = bp_pos_on_maze_filtered
    num_intervals = np.floor(trial_duration*60/interval).astype(int);
    
    head_dips_per_interval = np.zeros((num_intervals, 12))
    hole_visits_df = pd.DataFrame(columns = ['Box','ID','Group','Day','Trial','Interval','Hole_visits'])
    
    # For probe trials:
    if(probe):
        # Calculate the number of head dips for each interval
        for ii in range(num_intervals):
            
            # Get only the head dips in the interval
            holes_visited_interv = holes_visited[(ii*interval)*fps:(ii+1)*interval*fps]
            # If the actual point and the previous one were on a hole, ignore this hole and go to the next one
            if(ii > 0 and holes_visited_interv[0] > 0 and holes_visited[(ii*interval)*fps-1] > 0):
                next_0_index = np.where(holes_visited_interv == 0)
                holes_visited_interv = holes_visited_interv[next_0_index[0][0]:len(holes_visited_interv)]
            
            # Points outside the maze = 0
            holes_visited_interv[holes_visited_interv<0] = 0
            # Count number of repeated ocurrencies (head dips for each hole)
            holes_visited_interv = [k for k,g in groupby(holes_visited_interv) if k!=0]
            hole_visits = [holes_visited_interv.count(k) for k in range(1,13)]
            data_holes = pd.DataFrame([[box, ID, group, day, trial,
                                        str(ii*interval)+'-'+str((ii+1)*interval), hole_visits]], 
                                columns = ['Box','ID','Group','Day','Trial','Interval','Hole_visits'])
            
            hole_visits_df = pd.concat([hole_visits_df, data_holes], ignore_index = True)  
    
    # Still to implement the case when it is not a probe trial...
    
    return hole_visits_df
