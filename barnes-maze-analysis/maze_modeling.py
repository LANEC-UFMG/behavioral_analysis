...
def get_hole_positions(dlc_data_frame, filename, prev_hole_position=None):
    ...
    if (filename.find('B01_10_G2_D16_T1') != -1):
        position_each_hole = np.insert(position_each_hole, [10], [821.83, 114.48], axis=0)
    elif (filename.find('B01_13_G3_D4_T2') != -1):
        ...
        position_each_hole = np.insert(position_each_hole, [8], [498.002, 	159.114], axis=0)
        position_each_hole = position_each_hole[0:len(position_each_hole)-1]
    elif (filename.find('B01_14_G3_D4_T3') != -1):
        position_each_hole = np.insert(position_each_hole, [9], [618.865,	 86.7905], axis=0)
        position_each_hole = position_each_hole[0:len(position_each_hole)-1]
    elif (filename.find('B01_14_G3_D16_T1') != -1):
        position_each_hole[1] = [920.991,	540.862]
    elif (filename.find('B01_18_G3_D2_T1') != -1):
        position_each_hole[10] = [820.948,	61.206]
    elif (filename.find('B01_19_G3_D1_T1') != -1):
        position_each_hole[10] = [814.998,	57.7557]
    elif (filename.find('B01_19_G3_D16_T1') != -1):
        position_each_hole[11] = [929.132,	190.758]
    elif (filename.find('B01_20_G3_D2_T1') != -1):
        position_each_hole[10] = [804.305,	54.709]
    elif (filename.find('B01_20_G3_D2_T3') != -1):
        position_each_hole[10] = [820.01, 60.6925]
    elif (filename.find('B01_20_G3_D3_T3') != -1):
        position_each_hole[9] = [683.375, 75.4044]        
    elif (filename.find('B01_25_G1_D3_T3') != -1):
        position_each_hole[10] = [851.776, 115.618]
